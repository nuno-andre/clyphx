...
class ClyphXM4LBrowserInterface(ControlSurfaceComponent):
    __module__ = __name__
    __doc__ = '''ClyphXM4LBrowserInterface provides access to browser data and
    ...
    def get_items_for_device(self, device_name):
        """Returns the list of folders and items for the given device and
        stores the device.
        """
    ...
    def _create_devices_for_tag(self, tag):
        """Creates dict of devices for the given tag. Special handling is
        needed for M4L tag, which only contains folders, and Drums tag, which
        ontains devices and folders.
        """
        device_dict = {}
