...
LIVE_VER = 'Live v{}.{}.{}'.format(
    live_app.get_major_version(),
    live_app.get_minor_version(),
...
class InstantMappingMakeDoc(object):
    """InstantMappingMakeDoc creates a html file in the user's home directory
    containing information on the parameter banks defined for all Live devices
    in Devices.pyc.
    """
    ...
    def _write_device_info(self, file, info):
        ...
        for i, bn in enumerate(info['bank_names']):
            self._write_bank_parameters(file, 'B{}: {}'.format(i + 1, bn), info['banks'][i])
    ...
    def _write_bank_parameters(self, file, bank_name, bank):
        ...
        file.write('<b>{}</b><br>'.format(bank_name))
        for i, p in enumerate(bank):
            if p:
                file.write('P{}: {}<br>'.format(i + 1, p))
        file.write('<br>')
